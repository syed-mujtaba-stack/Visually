...
import { handleFaviconGeneration } from '@/app/actions';
...
export function FaviconGenerator() {
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    setIsLoading(true);
    setImageUrls(null);
    const result = await handleFaviconGeneration(data.prompt, data.count);
    setIsLoading(false);
    ...
    if (result.error) {
      ...
      toast({
        title: `Generated ${result.imageUrls.length} favicon(s)!`,
  ...
  const handleDownload = (url: string) => {
    ...
    link.href = url;
    link.download = `visually-generated-favicon-${Date.now()}.png`;
  ...
  const handleDownloadAll = () => {
    imageUrls?.forEach((url, index) => {
      const link = document.createElement('a');
      link.href = url;
      link.download = `visually-generated-favicon-${Date.now()}-${index + 1}.png`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
    });
  ...
  const shareOnTwitter = () => {
    const text = encodeURIComponent('Check out this favicon I generated with Visually! ✨');
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormControl>
                    <Input
                      placeholder="e.g., A minimalist 'V' for Visually"
                      className="text-base"
                      {...field}
                    />
                  </FormControl>
                ...
                render={({ field }) => (
                  <FormItem className="w-1/3">
                    <Select onValueChange={field.onChange} defaultValue={String(field.value)}>
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="Number of favicons" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                ...
                {isLoading ? (
                  ...
                  <div className="w-full aspect-square rounded-lg bg-black/20 border-2 border-dashed border-border flex items-center justify-center overflow-hidden">
                    <div className="flex flex-col items-center gap-4 text-muted-foreground">
                        <ImageIcon size={48} />
                        <p>Your generated favicon will appear here</p>
                    </div>
                  </div>
                )}
              </div>
